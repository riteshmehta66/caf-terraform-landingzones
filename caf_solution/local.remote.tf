locals {
  remote = {
    ...
    eventgrid_topics = {
      for key, value in try(var.landingzone.tfstates, {}) : key => merge(try(data.terraform_remote_state.remote[key].outputs.objects[key].eventgrid_topics, {}))
    }
    ...
    notification_hub_namespaces = {
      for key, value in try(var.landingzone.tfstates, {}) : key => merge(try(data.terraform_remote_state.remote[key].outputs.objects[key].notification_hub_namespaces, {}))
    }
    postgresql_servers = {
